...
# ### Tasks
#
# 0. (Make sure that you start a few worker processes.)
#
# 1. Based on `compute_pi`, write a function `compute_pi_parallel(N::Int)` which does the same but divides the work among the available worker processes.
#
...
function compute_pi_parallel(N::Int)
    ...
    sum_of_pis = @distributed (+) for i=1:nw
        compute_pi(ceil(Int, N / nw))
    end

    return sum_of_pis / nw  # average value
end

@btime compute_pi_parallel(10_000_000)
# -
